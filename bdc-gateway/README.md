...
| 端口 | `8080` |
| 主数据库 | — |
| Kafka Topic | `audit.gateway` |
| 包根 | `com.bdc.gateway` |
| 启动类 | `com.bdc.gateway.GatewayApplication` |
| 落地阶段 | **P2.2**（当前 P0.1.2 仅骨架） |

## 职责

网关：路由、鉴权、限流、国密签名校验、审计埋点

## 目录结构

```
bdc-gateway/
├── api/                # OpenAPI 3 规范（service-gateway.yaml）
├── sql/                # Flyway 脚本：V1.0.0__init.sql ...
├── docker/             # Dockerfile / 启动脚本
├── src/main/java/      # 源码：com.bdc.gateway.*
...
- [x] 目录骨架与设计文档一致
- [ ] `./gradlew :bdc-gateway:tasks` 能列出任务（待 P0.3.4）
- [ ] `./gradlew :bdc-gateway:bootRun` 启动后 `/actuator/health` 200（待 P2.2）