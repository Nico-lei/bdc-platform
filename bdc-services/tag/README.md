...
| Kafka Topic | `—` |
| 包根 | `com.bdc.tag` |
| 启动类 | `com.bdc.tag.TagApplication` |
| 落地阶段 | **P4.3**（当前 P0.1.2 仅骨架） |
...
```
bdc-services\tag/
├── api/                # OpenAPI 3 规范（service-tag.yaml）
├── sql/                # Flyway 脚本：V1.0.0__init.sql ...
├── docker/             # Dockerfile / 启动脚本
├── src/main/java/      # 源码：com.bdc.tag.*
...
- [x] 目录骨架与设计文档一致
- [ ] `./gradlew :bdc-services:tag:tasks` 能列出任务（待 P0.3.4）
- [ ] `./gradlew :bdc-services:tag:bootRun` 启动后 `/actuator/health` 200（待 P4.3）