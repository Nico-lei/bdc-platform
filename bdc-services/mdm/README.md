...
| 端口 | `8104` |
| 主数据库 | bdc_app.mdm |
| Kafka Topic | `mdm.sync` |
| 包根 | `com.bdc.mdm` |
| 启动类 | `com.bdc.mdm.MdmApplication` |
| 落地阶段 | **P4.1**（当前 P0.1.2 仅骨架） |

## 职责

主数据（师生/部门/课程/专业/组织/校历等）

## 目录结构

```
bdc-services\mdm/
├── api/                # OpenAPI 3 规范（service-mdm.yaml）
├── sql/                # Flyway 脚本：V1.0.0__init.sql ...
├── docker/             # Dockerfile / 启动脚本
├── src/main/java/      # 源码：com.bdc.mdm.*
...
- [x] 目录骨架与设计文档一致
- [ ] `./gradlew :bdc-services:mdm:tasks` 能列出任务（待 P0.3.4）
- [ ] `./gradlew :bdc-services:mdm:bootRun` 启动后 `/actuator/health` 200（待 P4.1）