...
| 端口 | `8101` |
| 主数据库 | bdc_app.auth |
| Kafka Topic | `event.auth.login` |
| 包根 | `com.bdc.auth` |
| 启动类 | `com.bdc.auth.AuthApplication` |
| 落地阶段 | **P2.2**（当前 P0.1.2 仅骨架） |

## 职责

认证：CAS/LDAP/OAuth2 回调、Token、三权分立管理

## 目录结构

```
bdc-services\auth/
├── api/                # OpenAPI 3 规范（service-auth.yaml）
├── sql/                # Flyway 脚本：V1.0.0__init.sql ...
├── docker/             # Dockerfile / 启动脚本
├── src/main/java/      # 源码：com.bdc.auth.*
...
- [x] 目录骨架与设计文档一致
- [ ] `./gradlew :bdc-services:auth:tasks` 能列出任务（待 P0.3.4）
- [ ] `./gradlew :bdc-services:auth:bootRun` 启动后 `/actuator/health` 200（待 P2.2）