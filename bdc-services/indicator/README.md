...
| 服务名 | `bdc-indicator` |
| 端口 | `8105` |
| 主数据库 | bdc_app.indicator |
| Kafka Topic | `—` |
| 包根 | `com.bdc.indicator` |
| 启动类 | `com.bdc.indicator.IndicatorApplication` |
| 落地阶段 | **P4.2**（当前 P0.1.2 仅骨架） |
...
```
bdc-services\indicator/
├── api/                # OpenAPI 3 规范（service-indicator.yaml）
├── sql/                # Flyway 脚本：V1.0.0__init.sql ...
├── docker/             # Dockerfile / 启动脚本
├── src/main/java/      # 源码：com.bdc.indicator.*
...
- [x] 目录骨架与设计文档一致
- [ ] `./gradlew :bdc-services:indicator:tasks` 能列出任务（待 P0.3.4）
- [ ] `./gradlew :bdc-services:indicator:bootRun` 启动后 `/actuator/health` 200（待 P4.2）