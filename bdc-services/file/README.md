...
| Kafka Topic | `—` |
| 包根 | `com.bdc.file` |
| 启动类 | `com.bdc.file.FileApplication` |
| 落地阶段 | **P4.10**（当前 P0.1.2 仅骨架） |

## 职责

文件上传下载 + MinIO 代理 + 水印嵌入

## 目录结构

```
bdc-services\file/
├── api/                # OpenAPI 3 规范（service-file.yaml）
├── sql/                # Flyway 脚本：V1.0.0__init.sql ...
├── docker/             # Dockerfile / 启动脚本
├── src/main/java/      # 源码：com.bdc.file.*
...
- [x] 目录骨架与设计文档一致
- [ ] `./gradlew :bdc-services:file:tasks` 能列出任务（待 P0.3.4）
- [ ] `./gradlew :bdc-services:file:bootRun` 启动后 `/actuator/health` 200（待 P4.10）