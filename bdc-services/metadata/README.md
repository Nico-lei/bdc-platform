...
| 服务名 | `bdc-metadata` |
| 端口 | `8103` |
| 主数据库 | bdc_app.metadata |
| Kafka Topic | `event.meta.change` |
| 包根 | `com.bdc.metadata` |
| 启动类 | `com.bdc.metadata.MetadataApplication` |
| 落地阶段 | **P2.4**（当前 P0.1.2 仅骨架） |

## 职责

数据资产目录、元数据、数据标准、数据地图

## 目录结构

```
bdc-services\metadata/
├── api/                # OpenAPI 3 规范（service-metadata.yaml）
├── sql/                # Flyway 脚本：V1.0.0__init.sql ...
├── docker/             # Dockerfile / 启动脚本
├── src/main/java/      # 源码：com.bdc.metadata.*
...
- [x] 目录骨架与设计文档一致
- [ ] `./gradlew :bdc-services:metadata:tasks` 能列出任务（待 P0.3.4）
- [ ] `./gradlew :bdc-services:metadata:bootRun` 启动后 `/actuator/health` 200（待 P2.4）