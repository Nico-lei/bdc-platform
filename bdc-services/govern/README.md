...
| 端口 | `8109` |
| 主数据库 | bdc_app.govern |
| Kafka Topic | `govern.quality` |
| 包根 | `com.bdc.govern` |
| 启动类 | `com.bdc.govern.GovernApplication` |
| 落地阶段 | **P3.3**（当前 P0.1.2 仅骨架） |
...
```
bdc-services\govern/
├── api/                # OpenAPI 3 规范（service-govern.yaml）
├── sql/                # Flyway 脚本：V1.0.0__init.sql ...
├── docker/             # Dockerfile / 启动脚本
├── src/main/java/      # 源码：com.bdc.govern.*
...
- [x] 目录骨架与设计文档一致
- [ ] `./gradlew :bdc-services:govern:tasks` 能列出任务（待 P0.3.4）
- [ ] `./gradlew :bdc-services:govern:bootRun` 启动后 `/actuator/health` 200（待 P3.3）