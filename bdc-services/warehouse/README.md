...
| 服务名 | `bdc-warehouse` |
| 端口 | `8108` |
| 主数据库 | bdc_app.warehouse |
| Kafka Topic | `—` |
| 包根 | `com.bdc.warehouse` |
| 启动类 | `com.bdc.warehouse.WarehouseApplication` |
| 落地阶段 | **P3.2**（当前 P0.1.2 仅骨架） |
...
```
bdc-services\warehouse/
├── api/                # OpenAPI 3 规范（service-warehouse.yaml）
├── sql/                # Flyway 脚本：V1.0.0__init.sql ...
├── docker/             # Dockerfile / 启动脚本
├── src/main/java/      # 源码：com.bdc.warehouse.*
...
- [x] 目录骨架与设计文档一致
- [ ] `./gradlew :bdc-services:warehouse:tasks` 能列出任务（待 P0.3.4）
- [ ] `./gradlew :bdc-services:warehouse:bootRun` 启动后 `/actuator/health` 200（待 P3.2）