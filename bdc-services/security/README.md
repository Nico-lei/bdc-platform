...
| 服务名 | `bdc-security` |
| 端口 | `8115` |
| 主数据库 | bdc_app.security |
| Kafka Topic | `security.alert` |
| 包根 | `com.bdc.security` |
| 启动类 | `com.bdc.security.SecurityApplication` |
| 落地阶段 | **P5.1**（当前 P0.1.2 仅骨架） |
...
```
bdc-services\security/
├── api/                # OpenAPI 3 规范（service-security.yaml）
├── sql/                # Flyway 脚本：V1.0.0__init.sql ...
├── docker/             # Dockerfile / 启动脚本
├── src/main/java/      # 源码：com.bdc.security.*
...
- [x] 目录骨架与设计文档一致
- [ ] `./gradlew :bdc-services:security:tasks` 能列出任务（待 P0.3.4）
- [ ] `./gradlew :bdc-services:security:bootRun` 启动后 `/actuator/health` 200（待 P5.1）