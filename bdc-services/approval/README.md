...
| 端口 | `8113` |
| 主数据库 | bdc_flowable |
| Kafka Topic | `approval.*` |
| 包根 | `com.bdc.approval` |
| 启动类 | `com.bdc.approval.ApprovalApplication` |
| 落地阶段 | **P4.6**（当前 P0.1.2 仅骨架） |

## 职责

审批：Flowable 引擎、流程定义、节点配置

## 目录结构

```
bdc-services\approval/
├── api/                # OpenAPI 3 规范（service-approval.yaml）
├── sql/                # Flyway 脚本：V1.0.0__init.sql ...
├── docker/             # Dockerfile / 启动脚本
├── src/main/java/      # 源码：com.bdc.approval.*
...
- [x] 目录骨架与设计文档一致
- [ ] `./gradlew :bdc-services:approval:tasks` 能列出任务（待 P0.3.4）
- [ ] `./gradlew :bdc-services:approval:bootRun` 启动后 `/actuator/health` 200（待 P4.6）